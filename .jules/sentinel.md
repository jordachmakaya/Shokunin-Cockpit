...
## 2025-05-15 - js-yaml v4 Compatibility with gray-matter
**Vulnerability:** Application crash when forcing `js-yaml` v4 via `pnpm.overrides`.
**Learning:** `gray-matter` internally expects `js-yaml` v3 API (`safeLoad`). When security overrides force `js-yaml` v4, `gray-matter` crashes because `safeLoad` was removed.
**Prevention:** When forcing `js-yaml` v4, explicitly configure `gray-matter` engines to use `yaml.load` (which is safe by default in v4) and add `js-yaml` as a direct dependency to ensure API availability.
